...
<body>
 ...
 <div class="loginbox" id="logindiv">
    <img src="../umrago pics/avatar.png" class="avatar">
     ...
     <h1>Verify Otp</h1>
     <label><?php  echo "Otp Successfully send to   " . $_SESSION['contact_org_session']; ?></label>
        <form class="form-signin" method="post" action="ver_org.php">
...
if(isset($_POST['verify_ins']))
    {
                                        $otp = $_POST['otp'];
                               $full_name   =  $_SESSION['full_name_org_session'];
                                $email   =  $_SESSION['email_org_session'];
                                $password   =   $_SESSION['password_org_session'];
                                 $contact   = $_SESSION['contact_org_session'];
                                $company_name   =  $_SESSION['company_name_session'];
                                        if($_SESSION['otp_org'] == $otp) {
                                             echo "<script>alert('Your Mobile Number is verified')</script>";
                                              $query = "INSERT INTO organizer(organizer_name,organizer_contact,organizer_email,organizer_companyname,organizer_password) VALUES('$full_name','$contact','$email','$company_name','$password')";
